...
class ArtConverter:
    def __init__(self, path='img/car.jpg', font_size=10, color_lvl=8):
        pg.init()
        self.path = path
    ...
    def draw_converted_image(self):
        ...
        for y in range(0, self.HEIGHT - self.CHAR_STEP, self.CHAR_STEP):
            for x in range(0, self.WIDTH - self.CHAR_STEP, self.CHAR_STEP):
                char_index = int(char_indices[y, x])
                if char_index:
    ...
    def save_image(self):
        ...
        cv2.imwrite('output\img\converted_ascii_image.png', cv2_img)
